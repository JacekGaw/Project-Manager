...
const ProjectView = () => {
  const { projectDelete, addTask, showProject } = useContext(ProjectsContext);
  const modal = useRef();
  ...
  const handleAddTask = () => {
    ...
    newTask.length > 0
      ? addTask(showProject.id, newTask)
      : modal.current.open();
  };
    ...
    <>
      <Modal ref={modal}>
        ...
        <p className="text-slate-700 mb-5">
          Empty task? If you have nothing to do, just don't add one. If you
          have, please fill input!
        </p>
      </Modal>
      <section class="flex flex-col flex-1 w-full p-20">
        <header class="flex justify-between mb-2">
          <h2 class="text-4xl font-bold text-dark-blue">{showProject.title}</h2>
          <button
            class="p-2 mx-2 hover:shadow-md hover:-translate-y-0.5 transition"
            onClick={() => projectDelete(showProject)}
          >
            Delete
          </button>
        </header>
        <p class="text-light-blue mb-5">{showProject.date}</p>
        ...
        <Tasks projectId={showProject.id} />
      </section>
    </>
  );
};
